<template>
  <div id="calendar">
    ...
    <ul class="days">
      ...
      <li v-for="(dayobject, index) in days" :key="index" style="height: 120px;" @click="clickcourse($event,index)">
        <!--本月-->
        <!--如果不是本月  改变类名加灰色-->
        ...
        <!--显示剩余多少数量-->
        <!--<p v-if="leftobj[dayobject.index]">剩余：<span style="color: red" >{{leftobj[dayobject.index].count}}</span>-->
        <!--</p> -->
        <p v-if="leftobj[dayobject.index]">瑜伽
        </p>
        <!---->
...
export default {
  ...
  methods: {
    clickcourse: function (event, index) {
